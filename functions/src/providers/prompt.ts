...
Você deve responder chamando exclusivamente a ferramenta "submit_analysis" uma única vez, com um
objeto JSON que respeita o schema fornecido. Não escreva texto fora da chamada de ferramenta.
Preencha todos os campos obrigatórios do schema. Onde o schema permitir arrays vazios, é aceitável
retornar um array vazio quando genuinamente não houver nada a reportar naquela categoria — mas não
deixe de tentar identificar ao menos as referências bíblicas e afirmações teológicas mais
...
Gere a análise completa chamando a ferramenta submit_analysis. Use os ids de seção fornecidos
entre colchetes (ex.: "${sections[0]?.id ?? "sec-1"}") no campo sectionId sempre que um achado for
localizado em uma seção específica.`;
}
